...
- (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    if (blurEnabled)
    {
        for (LDCFXBlurView *view in self.views)
        {
...
- (void)addView:(LDCFXBlurView *)view
{
...
- (void)removeView:(LDCFXBlurView *)view
{
...
- (void)updateAsynchronously
{
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        {
            LDCFXBlurView *view = self.views[i];
